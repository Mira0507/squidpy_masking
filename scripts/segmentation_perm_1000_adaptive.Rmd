...
We rerun thresholding to overcome uneven thresholding when performed using the Otsu method. The 
[adaptive (aka local) thresholding](https://scikit-image.org/docs/0.25.x/auto_examples/applications/plot_thresholding_guide.html#local-thresholding) is performed for more uniform thresholding throughout the pixels,
using the `threshold_local` function provided in the `scikit-image` package. Threshold values are
...
def adaptive_thresh(block_size, offset, channel):
    ...
    img.add_img(mask.astype(np.uint8), layer=f"adaptive_channel_{channel}")
    
    # print and save
    plt.imshow(mask, cmap=ListedColormap(['black', 'white']))
    plt.xticks([])
    plt.yticks([])
    plt.savefig(f"{outdir}/adaptive_thresh_channel_{channel}.png", dpi=1000)
    plt.show()
...
i = 0
adaptive_thresh(block_size=block_size, offset=offset, channel=i)
outfile = f"{outdir}/adaptive_thresh_channel_{i}.png"
```

- Download: [`r py$outdir`/`r py$outfile`](`r py$outdir`/`r py$outfile`)
...
```{r }
# Extract all binarized array data
adaptive_lyrs = [c for c in list(img) if "adaptive" in c]
