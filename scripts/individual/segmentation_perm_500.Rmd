...
```{python add_binary_array}

# ------------------------------------------------------------------------------
# This chunk adds a binary array to the `ImageContainer` obj by filtering 
# segmentation labels.
# ------------------------------------------------------------------------------

# Prep a binary array storing masking information
binary_arr = stacked_arr > 0

# Add the binary array to the ImageConatiner obj
lyr_bn = f"{lyr}_bn"
try:
    img[lyr_bn] = binary_arr
except:
...
for (i in 1:py$img[[py$lyr]][['shape']][[4]]) { 
    ...
    t_deparsed_2 <- paste0("plt.savefig('", py$outdir, "/bn_channel_", i, ".png', dpi=1000)")
    t_deparsed <- c(t_deparsed_1, t_deparsed_2)
    subchunkify(name=paste0("bn_images_channel_", i), t_deparsed=t_deparsed)

    path <- paste0(py$outdir, "/bn_channel_", i, ".png")
    link_output(path)
    cat('\n\n')
}
...
# Comparison of row-wise intensities after each process {.tabset}

Here, we compare the intensity of each row (from top to bottom) across the channels at each step.
Refer to the following layers and corresponding image processing steps:

- `image`: input
- `image_smooth`: input + Gaussian smoothing
- `image_sqseg`: input + Gaussian smoothing + Otsu thresholding + Watershed segmentation
...
for (i in 1:py$img[[py$lyr]][['shape']][[4]]) { 
    ...
    for (l in py$all_lyrs) {
        ...
        subchunkify(name=paste0("input_intensities_", i, "_", l),
...
# Comparison of pixel-wise intensities after each process {.tabset}

```{python pixel_intensity}
# reticulate::repl_python()
...
```{r vis_pixel_intensities, results='asis'}
...
for (i in 1:py$img[[py$lyr]][['shape']][[4]]) { 
    ...
    for (l in py$all_lyrs) {
        cat('\n\n### Layer:', l, '\n\n')
        sm_rows <- py$all_lyrs
        t_deparsed_1 <- paste0("plt.plot(idic['", l, "'][", i, "],
                               marker='o',
                               linestyle='None',
                               color='blue')")
        t_deparsed_2 <- "plt.xlabel('Pixel number')"
        t_deparsed_3 <- "plt.ylabel('Pixel-wise intensity')"
        t_deparsed_4 <- "plt.show()"
        t_deparsed <- c(t_deparsed_1, t_deparsed_2, t_deparsed_3, t_deparsed_4)
        subchunkify(name=paste0("input_pixel_intensities_", i, "_", l),
                    t_deparsed=t_deparsed,
                    width=8,
                    height=6)
    }
...
input_arr = [np.squeeze(img[lyr_bn][:, :, :, i]) for i in range(img[lyr_bn].shape[-1])]
...
```{r vis_merge, results='asis'}
for (i in seq_along(py$merge_list)) {
    # Retrieve merge name in R
    merge_name <- py$merge_list[i]
    # Convert into 0-indexing
    i <- i - 1
    cat('\n\n## Merge:', merge_name, '\n\n')
    t_deparsed <- paste0("display_merge(merge_list[", i, "])\n\n")
    subchunkify(name=paste0("merge_", i), t_deparsed=t_deparsed)

    save_path <- paste0(py$outdir, "/", merge_name, ".png")
