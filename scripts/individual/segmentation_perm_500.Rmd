...
# Intensities of input images {.tabset}

We explore the intensity of input images across the channels.

```{python prep_input_arrays}

# --------------------------------------------------------------------------------
# This chunk prepares input 2D arrays across the channels
# --------------------------------------------------------------------------------

# reticulate::repl_python()

# Prep 2D arrays from the `ImageContainer` obj
input_arrays = [np.squeeze(img[lyr][:, :, :, i]) for i in range(img[lyr].shape[-1])]
summed_rows = [a.sum(axis=1) for a in input_arrays]
```

```{r vis_input_intensities, results='asis'}

# --------------------------------------------------------------------------------
# This chunk visualizes the distribution of pixel-wise intensities using lineplots
# --------------------------------------------------------------------------------
# Plot the intensities

for (i in seq_along(py$summed_rows)) { 
    i <- i - 1
    cat('## Channel', i, '\n\n')

    t_deparsed_1 <- paste0("plt.plot(summed_rows[", i, "], linestyle='-')")
...
# Intensities after segmentation
...
sum_arr = [a.sum(axis=1) for a in binary_arr]

for i, a in enumerate(sum_arr):
    print(f"Chennel {i}")
...
toy_arr = np.array([[0, 0, 0], [1, 1, 1], [2, 2, 2]])
print(toy_arr.sum(axis=0))
print(toy_arr.sum(axis=1))
```
