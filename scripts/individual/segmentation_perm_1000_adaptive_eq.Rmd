...
outdir = "segmentation_perm_1000_adaptive_eq"
...
# Custom Otsu thresholding {.tabset}

Alternatively, we conduct Otsu thresholding using the `threshold_otsu` function from the `scikit-image`
package.

```{python prep_otsu_thresholding}

# --------------------------------------------------------------------------------
# This chunk prepares a function to run otsu thresholding
# --------------------------------------------------------------------------------

def otsu_thresh(channel, erosion=True):
    # Extract smoothened array per channel
    smth = img[lyr_smth].values[:, :, 0, channel]
    
    # Run local thresholding
    otsu = threshold_otsu(smth)
    mask = smth > otsu
...
```{python otsu_dapi, fig.height=12, fig.width=12}
i = 0
otsu_thresh(channel=i, erosion=False)
outfile = f"{outdir}/ostu_channel_{i}.png"
```
...
```{python otsu_iba, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=False)
outfile = f"{outdir}/ostu_channel_{i}.png"
```
...
```{python otsu_tdp, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=False)
outfile = f"{outdir}/ostu_channel_{i}.png"
```
...
```{python otsu_map, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=False)
outfile = f"{outdir}/ostu_channel_{i}.png"
```
...
```{python otsu_aldh, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=False)
outfile = f"{outdir}/ostu_channel_{i}.png"
```
...
```{python otsu_erosion_dapi, fig.height=12, fig.width=12}
i = 0
otsu_thresh(channel=i, erosion=True)
outfile = f"{outdir}/otsu_erosion_channel_{i}.png"
```
...
```{python otsu_erosion_iba, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=True)
outfile = f"{outdir}/otsu_erosion_channel_{i}.png"
```
...
```{python otsu_erosion_tdp, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=True)
outfile = f"{outdir}/otsu_erosion_channel_{i}.png"
```
...
```{python otsu_erosion_map, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=True)
outfile = f"{outdir}/otsu_erosion_channel_{i}.png"
```
...
```{python otsu_erosion_aldh, fig.height=12, fig.width=12}
i += 1
otsu_thresh(channel=i, erosion=True)
outfile = f"{outdir}/otsu_erosion_channel_{i}.png"
```
...
def compare_erosion(image, channel, keyword):
    # Filter layers before and after erosion for all channels
    lyr_compared = [l for l in list(image) if keyword in l]
...
```{python compare_dapi_otsu, fig.height=12, fig.width=12}
i = 0
compare_erosion(image=img, channel=i, keyword="otsu")
```

### IBA1

```{python compare_iba_otsu, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="otsu")
```
...
```{python compare_tdp_otsu, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="otsu")
```
...
```{python compare_map_otsu, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="otsu")
```
...
```{python compare_aldh_otsu, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="otsu")
```
...
```{python compare_dapi_adaptive, fig.height=12, fig.width=12}
i = 0
compare_erosion(image=img, channel=i, keyword="adaptive")
```

### IBA1

```{python compare_iba_adaptive, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="adaptive")
```
...
```{python compare_tdp_adaptive, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="adaptive")
```
...
```{python compare_map_adaptive, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="adaptive")
```
...
```{python compare_aldh_adaptive, fig.height=12, fig.width=12}
i += 1
compare_erosion(image=img, channel=i, keyword="adaptive")
```
...
def display_merge(input_layers, merge_name, method):

    # Convert array dimension
    binary_arr = [np.squeeze(img[l].values) for l in input_layers]
...
```{python print_otsu_layers}
# Prep layer names of interest
input_lyrs = [l for l in list(img) if "otsu" in l and "erosion" not in l]
print(input_lyrs)
```

### DAPI + TDP43

```{python otsu_merge0, fig.width=12, fig.height=12}
method = "otsu"
i = 0
merge_name = merge_list[i]
display_merge(input_layers=input_lyrs, merge_name=merge_name, method=method)
...
```{python otsu_merge1, fig.width=12, fig.height=12}
i += 1
merge_name = merge_list[i]
display_merge(input_layers=input_lyrs, merge_name=merge_name, method=method)
outfile = f"{outdir}/{method}_{merge_name}.png"
...
```{python otsu_merge2, fig.width=12, fig.height=12}
i += 1
merge_name = merge_list[i]
display_merge(input_layers=input_lyrs, merge_name=merge_name, method=method)
outfile = f"{outdir}/{method}_{merge_name}.png"
...
```{python print_adaptive_layers}
# Prep layer names of interest
input_lyrs = [l for l in list(img) if "adaptive" in l and "erosion" in l]
print(input_lyrs)
```
