...
def image_original_smooth(image, image_cropped, channel):
    fig, axes = plt.subplots(nrows=2, ncols=2)
    image.show(layer=lyr, ax=axes[0, 0], channel=channel, cmap='gray')
    ...
    image_cropped.show(layer=lyr, ax=axes[1, 0], channel=channel, cmap='gray')
...
sq.im.segment(img=img, layer=lyr_smth, method=seg_method, thresh=th_method)
```
