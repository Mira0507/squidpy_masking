...
# Load adata obj corresponding to the fluorescence image
...
Images are compared between the before (left) and the after (right) segmentation, as well as the original 
(upper) and higher (lower) magnification, across the stainings.
...
```{python napari_exploration, eval=TRUE}

# Initialize napari viewer
viewer = napari.Viewer()

# Add layers of interest to napari viewer
viewer.add_image(img.data['seg_channel_0'].values)
viewer.add_image(img.data['seg_channel_1'].values)
viewer.add_image(img.data['seg_channel_2'].values)
