...
# Specify merges
merge_list = ['dapi_tdp', 'iba_map_aldh']
```
...
for m in merge_list:
    ...
    ch_list = list(m.split("_"))
    ch_dic = {ab: i for i, ab in enumerate(col_dic.keys()) if ab in ch_list}

    # Print merged binary images
    for ab, i in ch_dic.items():
        plt.imshow(binary_arr[i],
                   cmap=ListedColormap(['white', col_dic[ab]]),
                   alpha=0.5)
    plt.axis('off')
    plt.show()

    # # Prep merged arrays
    # arr = np.zeros((binary_arr[0].shape), dtype=int)
    # for i in range(len(ch_dic.keys())):
        # arr += binary_arr[i]
