...
# Path to watershed segmentation summary output
try:
    summary_out = params['summary_out']
except Exception:
    summary_out = ""

if summary_out is None or str(summary_out) in {"", "None", "NULL", "NA", "nan"}:
    summary_out = os.path.join(outdir, "watershed_summary.tsv")
...
# Collect compact per-channel watershed metrics while labels are in memory.
summary_rows = []
...
for ch in range(img[lyr].shape[3]):
    ...
    markers, marker_count = ndi.label(mask)
    ...
    input_mask = np.asarray(x_arr).astype(bool)
    label_values = np.asarray(labels)
    summary_rows.append({
        "sample": sample_name,
        "outdir": outdir,
        "channel": ch,
        "input_layer": lyr_processed,
        "watershed_layer": lyr_seg,
        "foreground_fraction": float(np.count_nonzero(input_mask) / input_mask.size) if input_mask.size else 0.0,
        "marker_count": int(marker_count),
        "label_count": int(np.count_nonzero(np.unique(label_values))),
...
try:
    ...
    raise ValueError("Can't add the stacked array to ImageContainer. Wrong dimension!")
...
print("Watershed summary written:", summary_out)
print(summary_df)

```

```{r watershed_summary, results='asis'}
