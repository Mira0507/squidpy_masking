...
crop_coord = {'width' : 10078, 'height' : 21579, 'size' : 820, 'scale' : 1}
...
# Specify config for rotation if necessary
rot = True
rot_angle = -90
```
...
def display_merge(merge_name, method):
    ...
    for ab, i in ch_dic.items():
        ...
        # Rotate if necessary
        if rot:
            ar = rotate(binary_arr[i], angle=rot_angle)
        plt.imshow(ar,
...
```{python rotation_config}
if rot:
    print(f"Masking image rotated: {rot_angle} degrees")
```
