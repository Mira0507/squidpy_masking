...
In the previous thresholding, the Otsu method, 

```{python prep_adaptive_thresholding}

# --------------------------------------------------------------------------------
# This chunk runs image masking using adaptive thresholding
# --------------------------------------------------------------------------------
...
def display_adaptive_thresh(i):
    color = list(col_dic.values())[i]
    
    dapi = img['image'].values[:, :, 0, i]
    
    adaptive_thresh = threshold_local(dapi, block_size=block_size, offset=offset)
    mask = dapi > adaptive_thresh
    
    plt.imshow(mask, cmap=ListedColormap([(1, 1, 1, 0), color]), alpha=0.5)
    plt.xticks([])  # Remove x-axis ticks
    plt.yticks([])  # Remove y-axis ticks
    plt.savefig(f"{outdir}/adaptive_thresh_channel_{i}.png", dpi=1000)
    plt.show()
